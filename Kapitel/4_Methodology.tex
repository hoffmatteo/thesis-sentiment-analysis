...
\section{Machine Learning Method}
    \subsection{Naive Bayes}
        ...
        \begin{equation}
            ...
            P(d|c) = \prod_{i=1}^{n}P(d_{i}|y)
        \end{equation}
        with $d$ containing $n$ attributes $\{d_1,d_2,...,d_3\}$ \cite{DBLP:books/aw/TanSKK2019}.

        Furthermore, $P(d)$ remains constant for every class label $c$, so the class that maximizes the following is chosen \cite{DBLP:books/aw/TanSKK2019}:
        \begin{equation}
            \label{eq:naive_final}
            P(c|d)\propto P(d)\prod_{i=1}^{n}P(d_{i}|c) 
        \end{equation}   
        ...
        According to Forbes et al., "the multinomial variate is a multidimensional generalization of the binomial. Consider a trial that can result in only one of $k$ possible distinct outcomes, labeled $A_i, i = 1,...,k$. Outcome $A_i$ occurs with probability $pi$. The multinomial distribution relates to a set of n-independent trials of this type." \cite[p.~135]{evans2011statistical}.
        ...
        In order to estimate $P(w_t|c_j$, the probability of a word $w_t$ given a class $c_j$, the training instances are used to count the amount of times a word appears in a class:
        \begin{equation}
            \label{eq:prob_word}
                P(w_t|c_j) = \frac{1 + \sum_{i=1}^{|D|}N_{it} + P(c_j|d_i)}{|V| + \sum_{s=1}^{|V|} \sum_{i=1}^{|D|}N_{is} P (c_j|d_i)} 
        \end{equation}
        with $D$ containing labeled training documents $d_i$, $N_{it}$ describing the number of times $w_t$ appears in the document and $|V|$ containing all words.
        Because the class conditional probabilities for labeled instances is 1 for one instance, and 0 for the other ones, \eqref{eq:prob_word} can be simplified:
                \begin{equation}
            \label{eq:prob_word_simpler}
                P(w_t|c_j) = \frac{1 + N_{tj}}{|V| + \sum_{s=1}^{|V|}N_{sj}} 
        \end{equation}
        \TODO{evtl. anderes N, erklären, etc.}
